value_max(20).

% {in_knapsack(X, V): V=1..MAX, value_max(MAX)} = 1 :- object(X,WEIGHT,VALUE).
{in_knapsack(X, V): V=2..4, value_max(MAX)} = 1 :- object(X,WEIGHT,VALUE).

#eosum{V*WEIGHT : in_knapsack(X,V), object(X,WEIGHT,_)[X]} <= MAX_WEIGHT: ub(MAX_WEIGHT,0).
% :- #sum{V*WEIGHT,X: in_knapsack(X,V), object(X,WEIGHT,_)} > MAX_WEIGHT, ub(MAX_WEIGHT,0).

#eosum{ V*VALUE : in_knapsack(X,V), object(X,_,VALUE)[X] } >= MIN : lb(MIN,1).
% :- #sum{V*VALUE,X: in_knapsack(X,V), object(X,_,VALUE)} < MIN_VALUE, lb(MIN_VALUE,1).