...
% turn(id, name, hours).
turn(1,"1-morning",7).
turn(2,"2-afternoon",7).
turn(3,"3-night",10).
turn(4,"4-restafternights",0).
turn(5,"5-rest",0). %called weekend in the document.
turn(6,"6-holiday",0).

%%%%% encoding %%%%%

% Choose an assignment for each day and for each nurse.
1 <= {assign(N, T, D) : turn(T,_,_)} <= 1 :- day(D), nurse(N).

% Each nurse works from 1687 to 1692 hours per year.
:- nurse(N), maxHoursPerYear(MAX), #sum{H,D : assign(N,T,D), turn(T,_,H)} > MAX.
:- nurse(N), minHoursPerYear(MIN), #sum{H,D : assign(N,T,D), turn(T,_,H)} < MIN.
...
% Fair distribution (morning, afternoon, night)
% ---> morning
:~ nurse(N), #count{D : assign(N,1,D)} = X, minDays(MIN), maxDays(MAX), X >= MIN, X <= MAX, Y=|X-78|. [Y@1,N]
% ---> afternoon
:~ nurse(N), #count{D : assign(N,2,D)} = X, minDays(MIN), maxDays(MAX), X >= MIN, X <= MAX, Y=|X-78|. [Y@1,N]
% ---> night
:~ nurse(N), #count{D : assign(N,3,D)} = X, minNights(MIN), maxNights(MAX), X >= MIN, X <= MAX, Y=|X-60|. [Y@1,N]
