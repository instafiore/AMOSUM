...
valid_excecutables = [r"plain", r"ijcai.*wasp", r"ijcai.*-c$", r"D-clingo.*-c(-|\.)", r"D-wasp-amosum-amo-py-full", r"D-wasp-amosum-eo-py-full"]
avoid_excecutables = [r"hybrid"]
...
for version, version_group in versions:
    ...
    for executable, exec_group in executables:
        if not any([True for valid in valid_excecutables if re.search(valid, executable[0])]):
            continue
        ...
        if start:
            print(f"\\multirow{{{len(version_group)}}}{{*}}{{\\rotatebox{{90}}{{{version[0]}}}}} & {executable[0]} & ", end="")
            start = False
        else:
