...
for n in range(start_n,end_n+1,5):
    ...
    C1_v =  n*k*mean_object_value # Setti a vero tutti i massimi (Possible sat)
    
    C2_v =  n * mean_object_value * (k * (k+1)) / 2 # Setti a vero tutti (Unsat)
    ...
    for i in range(nips):
        ...
        if i < size_1:
            ...
            lbs_value = int(np.random.uniform(0, C)) # Verso C miglioriamo

        elif i < size_2:

            type = "type3" 
            lbs_value = int(np.random.uniform(C1_v*0.10, C1_v*1.1)) # Verso C1 miglioriamo
