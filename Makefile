...
cxxflags.release = -Wall -Wextra -std=c++11 -DNDEBUG -O3 -I$(CONDA_PREFIX)/include
linkflags.release = -lm -ldl -L$(CONDA_PREFIX)/lib -Wl,-rpath,$(CONDA_PREFIX)/lib
...
scriptsc.python38 = -I$(CONDA_PREFIX)/include $(shell $(CONDA_PREFIX)/bin/python3-config --cflags | sed "s/-Wshorten-64-to-32//g" | sed "s/-Wstrict-prototypes//g" | sed "s/-arch i386//g" | sed "s/-DNDEBUG//g" | sed "s/-Os//g" )
scriptsld.python38 = $(shell $(CONDA_PREFIX)/bin/python3-config --ldflags --embed | sed "s/-Wshorten-64-to-32//g" | sed "s/-Wstrict-prototypes//g")
