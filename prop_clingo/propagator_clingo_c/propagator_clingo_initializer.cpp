...
bool PropagatorClingoInitializer::init(clingo_propagate_init_t *init, PropagatorClingo& propagator){

    if(!first){
        propagator.atomNames = atomNames.get();
    ...
    atomNames.reset(new std::unordered_map<clingo_symbol_t, clingo_literal_t>());
    ...
    nt = clingo_propagate_init_number_of_threads(init);
